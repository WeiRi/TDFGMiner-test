...
import common.collection.Function3;
import common.collection.Function4;
import javaimpl.cfg.CFG;
import javaimpl.cfg.CFGStatements;
import javaimpl.ir.IRExpression;
import javaimpl.ir.expressions.*;
import javaimpl.ir.statements.*;
...
public class JavaExpressionVisitor extends GenericVisitorWithDefaults<Optional<IRExpression>, CFGStatements> {
    ...
    //此处利用supplier机制，返回表达式中的类型
    //TODO：问题根源，部分表达式会报运行错误，原因似乎是没有处理递归调用？无法处理“Jsoup.parse(html)”，但是可以处理Parser.htmlParser()?
    private Optional<String> typeOf(Expression expr) {
//        if(expr.isMethodCallExpr()){
//            MethodCallExpr methodCallExpr=expr.asMethodCallExpr();
//            while (methodCallExpr.hasScope()){
//                methodCallExpr= methodCallExpr.getScope().get().asMethodCallExpr();
//            }
//            return TryUtil.optionalTry(() -> (((MethodCallExpr) expr).getScope().get().asMethodCallExpr()).getScope().get().calculateResolvedType().describe());
//        }else
        return TryUtil.optionalTry(() -> expr.calculateResolvedType().describe());
    }

    //TODO：不知为何无法解析“String”类
    ...
    public Optional<IRExpression> visit(MethodCallExpr n, CFGStatements arg) {
        //TODO:问题似乎出在这里，一些方法调用没有执行resolveMethodCallExpr函数
